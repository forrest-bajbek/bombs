...
func (h *Handler) UserInvitePartialLink(w http.ResponseWriter, r *http.Request) {
	err := r.ParseForm()
	if err != nil {
		component := components.PartialUserInviteError(err.Error())
		component.Render(context.Background(), w)
		return
	}
	...
	if user_exists {
		component := components.PartialUserInviteError("Username already exists!")
		component.Render(context.Background(), w)
		return
	}
...
func (h *Handler) UserCreatePage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, "Invalid Invite Token", 400)
		return
	}
...
func (h *Handler) UserCreate(w http.ResponseWriter, r *http.Request) {
	...
	inviteClaims, err := h.tokenMaker.ValidateInviteToken(inviteToken)
	if err != nil {
		component := components.UserCreatePage(username, inviteToken, "Invalid Invite Token")
		component.Render(context.Background(), w)
		return
	}
	...
	if err != nil {
		component := components.UserCreatePage(username, inviteToken, err.Error())
		component.Render(context.Background(), w)
		return
	}
